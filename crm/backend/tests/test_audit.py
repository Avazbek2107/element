...
def test_superadmin_user_creation_is_logged(client, db_session):
    ...
    cookies = auth_cookies(super_admin)
    ...
    logs = client.get("/api/audit-logs", cookies=cookies)
...
def test_audit_log_forbidden_for_non_super_admin(client, db_session):
    ...
    resp = client.get("/api/audit-logs", cookies=auth_cookies(admin))
...
def test_permission_change_is_logged_with_details(client, db_session):
    ...
    logs = client.get("/api/audit-logs", params={"action": "permission_change"}, cookies=cookies)
    items = logs.json()["items"]
    assert len(items) == 1
